...
func doAction(_context:PawnActionContext) -> bool:
	...
	if(theEntry):
		GM.actionSystem.denyAction(theEntry, _context.pawn)
		GM.pawnRegistry.addHoverTextGlobal(_context.pawn, "{user.You} {user.youVerb deny|denies}!", {user=_context.pawn.getCharID()})
	return true
