...
func doAction(_context:PawnActionContext) -> bool:
	...
	var theCurrentLeash := GM.leashSystem.getLeash(
		LeashPointConnection.createLeashpoint(_context.target), LeashPointConnection.createPawnLeashpoint(targetPawn, targetPawnPoint)
	)
	if(!theCurrentLeash):
		return false
